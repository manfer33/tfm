...
if __name__ == '__main__':
    ...
    Alpha = rospy.get_param("/armdrone/alpha")
    Epsilon = rospy.get_param("/armdrone/epsilon")
    Gamma = rospy.get_param("/armdrone/gamma")
    epsilon_discount = rospy.get_param("/armdrone/epsilon_discount")
    nepisodes = rospy.get_param("/armdrone/nepisodes")
    nsteps = rospy.get_param("/armdrone/nsteps")

    q_values = load_json("/home/manfer/tfm_ws/src/tfm/arm_training/training_results/old/14614.txt")

    # Initialises the algorithm that we are going to use for learning
    qlearn = qlearn.QLearn(actions=range(env.action_space.n), alpha=Alpha, gamma=Gamma, epsilon=Epsilon)
    qlearn.load_q_table(q_values)
